...
def tile(input: Tensor, kernel: Tuple[int, int]) -> Tuple[Tensor, int, int]:
    ...
    out = input.contiguous().view(batch, channel, new_height, kh, width)
...
def avgpool2d(input: Tensor, kernel: Tuple[int, int]) -> Tensor:
    ...
    return new_input.mean(dim=4).view(batch, channel, new_height, new_width)
...
def logsoftmax(input: Tensor, dim: int) -> Tensor:
    ...
    modified_input = input - max_elem
    sum_ = modified_input.exp().sum(dim).log()
    return input - max_elem - sum_
...
def maxpool2d(input: Tensor, kernel: Tuple[int, int]) -> Tensor:
    ...
    return max_reduce(new_input, 4).view(batch, channel, new_height, new_width)
